...
| comment_range_compose       | Viewer → host: the user pressed Comment. Open Activity if needed and check the timestamp toggle.                                                                                                            | Draft range: `{ startMs, endMs }` milliseconds, `endMs > startMs`. The host must not echo `comment_range_draft`. Paused with no range: `{ startMs }` only, at the playhead. |

The host also sends these events to the MP3 viewer (audio player v2 only). Checkbox-driven handles appear on `comment_range_draft`. A waveform drag can draw a range before that event; Comment then emits `comment_range_compose`. While playback is paused and no range is drawn, that same Comment button sits above the playhead and emits `comment_range_compose` with `{ startMs }` only.
